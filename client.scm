...
(define (m.room.member-printer evt ctx)
  (let* ((who (mref '(state_key) evt))
         ...
         (what (case (string->symbol (mref '(content membership) evt))
                 ...
                 ((join)
                  (cond ((member who (or (mref '(members) ctx) '()))
                         (let ((old-name (mref `(member-names ,who) ctx))
                               (old-avatar (mref `(member-avatars ,who) ctx))
                               (new-avatar (mref '(content avatar_url) evt)))
                           (cond ((not (string=? old-name name))
                                  (sprintf "is the new name of ~A" old-name))
                                 ((not (string=? old-avatar new-avatar))
