(module (ensemble libs bindings) (setlocale flock char-width)
(import scheme (chicken base) (chicken foreign))

(foreign-declare "#include <locale.h>")
(foreign-declare "#include <sys/file.h>")

;; TODO this should go into the utf8 egg
#>
// From https://www.cl.cam.ac.uk/~mgk25/ucs/wcwidth.c

// this implementation expects wchar_t to be an unicode codepoint
...
(define (char-width char)
  (let* ((wcwidth (foreign-lambda int "mk_wcwidth" unsigned-integer32))
         (char-code (char->integer char))
         (width (wcwidth char-code)))
    (if (< width 0)
        (error 'char-width "invalid character code" char-code)
        width)))
